...
public class CharacterStatsGUI extends GameGUI{
	...
	public void repaintCharacterStats(int characterIndex){
		/*Player player = Book.getInstance().getPlayers()[characterIndex];
		...
		for(int j = 0; j < amountOfAttributes; j++){
			...
			String statsToShow = att + "";
			if(j < 2){
				this.charactersStats[characterIndex][j].setBounds(420 + characterIndex*105, 50 + j * 15, 100, 150);
				statsToShow = statsToShow.concat("/" + player.getMaxHpAndMP()[j]);
			}
			else{
				this.charactersStats[characterIndex][j].setBounds(430 + characterIndex*105, 50 + j * 15, 100, 150);
			}
			
			this.charactersStats[characterIndex][j].setText(statsToShow);
			this.defaultScreen.add(this.charactersStats[characterIndex][j]);
		}	*/
	}
